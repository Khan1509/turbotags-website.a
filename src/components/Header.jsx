        ...
        <div className="flex justify-between items-center">
          <Link to="/" className="flex items-center gap-3 focus:outline-none focus:ring-2 focus:ring-[#6366f1] focus:ring-offset-2 rounded-lg p-2" aria-label="TurboTags - Go to homepage">
            <picture className="transition-transform hover:scale-110">
              <source media="(min-width: 1024px)" srcSet={`/turbotags_header_logo_1000x320.svg?t=${Date.now()}`} />
              <source media="(min-width: 768px)" srcSet={`/turbotags_header_logo_500x160.svg?t=${Date.now()}`} />
              <img
                src={`/turbotags_header_logo_250x80.svg?t=${Date.now()}`}
                alt="TurboTags Logo"
